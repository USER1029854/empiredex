# Pair contract — WBNB/ROOT (BNB Smart Chain (Binance))

- **Address:** `0x3Af4Cf7953cab46a1B4Af5F483c2aC81b994777e`
- **Chain:** BNB Smart Chain (Binance)
- **Explorer:** https://bscscan.com/address/0x3Af4Cf7953cab46a1B4Af5F483c2aC81b994777e
...
|---|---|---|---|
| token0 | WBNB `0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c` | `834000000066319811044` | 834.000000 |
| token1 | ROOT `0xF2F9889e797d2AAddc234f4f3027C62c28165af4` | `112361226709576610599` | 112.361227 |

- **LP total supply:** 303.224616 EMP-LP
- **Estimated USD value held:** **$1,015,486.08**
- **Pricing method:** est-2x-token0 (WBNB priced; ROOT unpriced by DefiLlama) — DefiLlama's own coins API (`coins.llama.fi/prices/current`) does not carry a price for the DEX-issued / bridged side of this pool, so USD value is estimated as 2× the priced side, which is exactly the convention DefiLlama's own `getUniTVL` helper uses for Uniswap-V2-style forks (this pricing methodology was validated: summing this same calculation across all 143 enumerated pairs on all 8 reachable chains reproduces 99.70% of DefiLlama's reported total protocol TVL — see the main README's Coverage Quantification section).
...
- Sourcify record: https://sourcify.dev/server/v2/contract/56/0x3Af4Cf7953cab46a1B4Af5F483c2aC81b994777e
...
ROOT is a bridged (Multichain/Anyswap-style) representation of Ethereum's ROOTDEX token on BSC; it is NOT an EmpireDEX-deployed contract, only the pair holding it is in scope.
