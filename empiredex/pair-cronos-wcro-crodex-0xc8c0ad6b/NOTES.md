# Pair contract — WCRO/CRODEX (Cronos)

- **Address:** `0xC8c0Ad6b6dfDb4d53629A770d0d76f7f9257BAA1`
- **Chain:** Cronos
- **Explorer:** https://cronoscan.com/address/0xC8c0Ad6b6dfDb4d53629A770d0d76f7f9257BAA1
...
|---|---|---|---|
| token0 | WCRO `0x5C7F8A570d578ED84E63fdFA7b1eE72dEae1AE23` | `661535549475940977631305` | 661,535.549476 |
| token1 | CRODEX `0xc83859C413a6bA5ca1620cD876c7E33a232c1C34` | `6647672590015662411678` | 6,647.672590 |

- **LP total supply:** 66,284.089946 EMP-LP
- **Estimated USD value held:** **$61,706.73**
- **Pricing method:** est-2x-token0 (WCRO priced; CRODEX unpriced by DefiLlama) — DefiLlama's own coins API (`coins.llama.fi/prices/current`) does not carry a price for the DEX-issued / bridged side of this pool, so USD value is estimated as 2× the priced side, which is exactly the convention DefiLlama's own `getUniTVL` helper uses for Uniswap-V2-style forks (this pricing methodology was validated: summing this same calculation across all 143 enumerated pairs on all 8 reachable chains reproduces 99.70% of DefiLlama's reported total protocol TVL — see the main README's Coverage Quantification section).
...
token1 (0xc83859c4...) is the SAME address used for EMPIRE/FDEX/ADEX/PDEX/XDEX on other chains (CREATE2 vanity deployment); on Cronos its on-chain symbol() is 'CRODEX'. This token contract itself separately qualifies for inclusion below (see token-cronos-crodex) because it still holds 99.16% of this very pair's LP supply, unclaimed. No free Cronos explorer API is available; decompiled with heimdall.
